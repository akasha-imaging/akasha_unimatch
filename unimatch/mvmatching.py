...
def compute_refcam_points_original(corr1, z, KL, KR, baseline, device='cuda'):
    height, width = corr1.shape[1], corr1.shape[2]
    cxdiff = KL[0, 2] - KR[0, 2]
    ...
    Z = KL[0, 0]*baseline/(Z - cxdiff)
    Y = Z*(Y - KL[1, 2])/KL[1, 1]
    X = Z*(X - KL[0, 2])/KL[0, 0]

    return torch.cat([X[..., None], 
                    Y[..., None], 
                    Z[..., None]], axis=-1)

def fuse_costs_original(corrs, calibs):
    # print('=======ORIGINAL COST FUSION=======')
    device = corrs[0].device
    master_KL, master_KR = calibs[0][0][0].clone(), calibs[0][0][1].clone()
    kh = 2160/corrs[0].shape[1]
    kw = 3840/corrs[0].shape[2]
    master_KL[0, :] /= kw
    master_KL[1, :] /= kh
    master_KR[0, :] /= kw
    master_KR[1, :] /= kh
    master_pose = calibs[0][1][0].clone()
    master_rt = calibs[0][1][0] @ torch.inverse(calibs[0][1][1])
    master_volume = corrs[0]
    master_baseline = torch.linalg.norm(master_rt[:3, 3], axis=-1)
    # _, H, _, _, W = master_volume.shape
    fin_volume = corrs[0].clone()
    for (i, corr) in enumerate(corrs[1:], start=1):
        _, H, W, _ = corr.shape
        temp_volume = torch.zeros_like(master_volume)
        for z in range(temp_volume.shape[-1]):
            KL = calibs[i][0][0].clone()
            KL[0, :] /= kw
            KL[1, :] /= kh
            KR = calibs[i][0][1].clone()
            KR[0, :] /= kw
            KR[1, :] /= kh
            poseL = calibs[i][1][0].clone()
            poseR = calibs[i][1][1].clone()
            baseline2 = torch.linalg.norm((poseL@torch.inverse(poseR))[:3, 3])
            cxdiff = KL[0, 2] - KR[0, 2]
            transL = calibs[i][1][0]@torch.inverse(calibs[0][1][0])
            points = compute_refcam_points_original(master_volume[...,z], z, master_KL, master_KR, master_baseline, device)
            points2 = points@transL[:3, :3].T + transL[:3, 3:].T
            uvs2 = points2@KL.T
            uvs2 = (uvs2/uvs2[..., 2:])
            uvs2[..., 1] = 2*torch.clip(uvs2[..., 1], 0, H-1)/(H-1)-1
            uvs2[..., 2] = uvs2[..., 2] * (KL[0, 0]*baseline2/(points2[..., 2])) + cxdiff
            uvs2[..., 2] = 2*torch.clip(uvs2[..., 0] - uvs2[..., 2], 0, W-1)/(W-1)-1
            uvs2[..., 0] = 2*torch.clip(uvs2[..., 0], 0, W-1)/(W-1)-1
            corr_reshaped = corr[None, :, :, :, :]
            # 3. find the values in corr2 and sum to corr1 
            temp_volume[0, :, :, z] = torch.nn.functional.grid_sample(corr_reshaped, uvs2[None, :, :, :, [2, 0, 1]].type(corr_reshaped.dtype), align_corners=True).squeeze()
        fin_volume += temp_volume
        # fin_volume = temp_volume
    # return fin_volume
    return fin_volume/len(corrs)
...
def global_correlation_softmax_stereo(
    ...
    corrs = []
    for (feature0, feature1) in zip(features_left, features_right): 
    ...
    correlation = fuse_costs_original(corrs, calibs)
